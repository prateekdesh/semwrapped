...
function cleanProgram(s: string): string {
...
function getAttPct(courseCode: string, data: StudentData): number | null {
...
function markLabel(mark: number, maxMark: number): string {
  if (maxMark <= 65) {
    // Internal marks (typically out of 60)
    if (mark >= maxMark * 0.834) return 'solid';      // ~50/60
    if (mark >= maxMark * 0.75)  return 'decent';     // ~45/60
    if (mark >= maxMark * 0.667) return 'just about'; // ~40/60
    return 'rough';
  }
  const p = mark / maxMark;
  if (p >= 0.91) return 'O grade';
  if (p >= 0.81) return 'A+';
  if (p >= 0.71) return 'A';
  if (p >= 0.61) return 'B+';
  return 'passing';
...
interface AttendanceInfo {
  overall: number;
  byCourse: { name: string; code: string; pct: number }[];
  hasData: boolean;
}

function computeAttendance(data: StudentData): AttendanceInfo {
...
function markPct(c: CourseDetail): number {
  return c.maxMark > 0 ? (c.mark / c.maxMark) * 100 : 0;
}

function getBest(courses: CourseDetail[]): CourseDetail {
  return courses.reduce((b, c) => markPct(c) > markPct(b) ? c : b, courses[0]);
}

function getWorst(courses: CourseDetail[]): CourseDetail {
  return courses.reduce((w, c) => markPct(c) < markPct(w) ? c : w, courses[0]);
}

interface AttendanceArchetype { title: string; sub: string; accent: string }
function attendanceArchetype(p: number): AttendanceArchetype {
  if (p >= 90) return {
    title: 'The Regular',
...
interface AlmostMoment { course: CourseDetail; markGap: number; label: string }
function getAlmostMoments(courses: CourseDetail[]): AlmostMoment[] {
  const out: AlmostMoment[] = [];
  for (const c of courses) {
    const targets: { mark: number; label: string }[] = c.maxMark <= 65
      ? [
...
interface Archetype { title: string; desc: string; accent: string }
function alterEgo(cgpa: number): Archetype {
  if (cgpa >= 9.0) return { title: 'The Quiet Threat', desc: "Nobody saw you coming. The group chat that excludes you is scared now.", accent: '#10b981' };
  if (cgpa >= 7.5) return { title: 'The Consistent One', desc: 'Not flashy. Not collapsing. Just reliably there — every semester. It\'s giving "I have a plan and I\'m not telling anyone."', accent: '#3b82f6' };
  if (cgpa >= 6.0) return { title: 'The Everyman', desc: 'Peak average. Statistically, you are most people. Own it. Most people are fine.', accent: '#f59e0b' };
...
function truncate(name: string, words = 4): string {
  const parts = name.split(' ');
  return parts.length > words ? parts.slice(0, words).join(' ') + '…' : name;
}

function faFirst(fa: string): string {
  // Get first name or "Dr. X" form from FA full name
  const parts = fa.trim().split(/\s+/);
  if (parts[0].match(/^(Dr|Prof|Mr|Ms|Mrs)\.?$/i)) return `${parts[0]}. ${parts[1] ?? ''}`.trim();
  return parts[0];
}
...
function schedPad(ctx: AudioContext, out: AudioNode, midi: number, t: number, dur: number): void {
  [[0, 'sine', 0.07], [7, 'triangle', 0.035], [-7, 'triangle', 0.035]].forEach(([det, type, vol]) => {
    const osc = ctx.createOscillator();
    const g = ctx.createGain();
    osc.type = type as OscillatorType;
    osc.frequency.value = noteHz(midi as number);
    osc.detune.value = (det as number) + (Math.random() - 0.5) * 5;
    g.gain.setValueAtTime(0, t);
    g.gain.linearRampToValueAtTime(vol as number, t + 0.55);
    g.gain.setValueAtTime(vol as number, t + dur - 0.9);
...
function schedKick(ctx: AudioContext, out: AudioNode, t: number): void {
  const osc = ctx.createOscillator();
  const g = ctx.createGain();
...
function schedHat(ctx: AudioContext, out: AudioNode, t: number, vol: number): void {
  ...
  src.connect(hpf); hpf.connect(g); g.connect(out);
  src.start(t); src.stop(t + 0.05);
}

function schedCycle(startAt: number): void {
  if (!_actx || !_mGain) return;
  const ctx = _actx;
  const lpf = ctx.createBiquadFilter();
  lpf.type = 'lowpass'; lpf.frequency.value = 3200; lpf.Q.value = 0.6;
  lpf.connect(_mGain);
  ...
  LOFI_PROG.forEach((chord, ci) => {
    ...
    for (let b = 0; b < 8; b++) {
      const bt = cs + b * BEAT;
      if (b % 2 === 0) schedKick(ctx, _mGain!, bt);
...
function startMusic(): void {
  ...
  if (!_actx) {
    _actx = new (window.AudioContext || (window as unknown as { webkitAudioContext: typeof AudioContext }).webkitAudioContext)();
  ...
  function loop(prev: number): void {
    if (!_musicRunning) return;
...
function renderSlide(i: number, d: StudentData): string {
...
// 0 — Cold Open
function s0(p: StudentProfile): string {
...
function s1(att: AttendanceInfo): string {
  if (!att.hasData) {
    return `
      <div class="slide-inner animate-in">
...
// 2 — Best Subject
function s2(d: StudentData): string {
  if (!d.courses.length) return emptySlide('best subject', 'No course data available.');
  ...
  let attLine = '';
  if (attPct !== null) {
    if (attPct < 75)       attLine = `You only showed up <strong>${Math.round(attPct)}%</strong> of the time. And still pulled this off. Terrifying, honestly.`;
    else if (attPct >= 90) attLine = `<strong>${Math.round(attPct)}%</strong> attendance too. You actually tried here. It shows.`;
    else                   attLine = `<strong>${Math.round(attPct)}%</strong> attendance — the bare minimum of effort, somehow above-average results.`;
  } else {
...
// 3 — Worst Subject
function s3(d: StudentData): string {
  if (!d.courses.length) return emptySlide('worst subject', 'No course data available.');
...
// 4 — Visual breakdown
function s4(courses: CourseDetail[], att: AttendanceInfo): string {
...
function s5(d: StudentData, att: AttendanceInfo): string {
...
// 6 — GPA Trajectory
function s6(rec: AcademicRecord): string {
  ...
  if (prev) {
    ...
    if (delta > 0.2) { trendText = `Up ${delta.toFixed(2)} from last sem. Actual growth. Don't ruin it by relaxing now.`; trendColor = '#10b981'; }
    else if (delta < -0.2) { trendText = `Down ${Math.abs(delta).toFixed(2)} from last sem. Not your best era. We're calling it a plot twist, not a trend.`; trendColor = '#f97316'; }
  }
...
// 7 — Almost Moments
function s7(courses: CourseDetail[]): string {
...
// 8 — Exam Preview
function s8(timetable: ExamEntry[], profile: StudentProfile): string {
...
function s9(rec: AcademicRecord, profile: StudentProfile): string {
...
// 10 — By the Numbers
function s10(d: StudentData, att: AttendanceInfo): string {
...
// 11 — The Closer
function s11(p: StudentProfile): string {
...
function emptySlide(title: string, msg: string): string {
  return `
    <div class="slide-inner animate-in">
...
function buildStories(data: StudentData): void {
...
function buildProgressBars(): void {
...
function refreshProgressBars(): void {
...
function goToSlide(target: number, instant = false): void {
...
function triggerAnim(slideEl: HTMLElement): void {
  const inner = slideEl.querySelector('.slide-inner');
  if (!inner) return;
  inner.classList.remove('animate-in');
  void (inner as HTMLElement).offsetHeight; // force reflow
...
function attachNavListeners(): void {
  ...
  document.addEventListener('keydown', (e) => {
    if ($('stories-screen').classList.contains('hidden')) return;
  ...
  ss.addEventListener('touchend', (e) => {
    ...
    if (Math.abs(dx) > 48) goToSlide(dx > 0 ? currentSlide + 1 : currentSlide - 1);
  });
}

function hookSlideButtons(): void {
...
async function handleStep1(e: Event): Promise<void> {
  e.preventDefault();
  hideError();

  const rawUsername = $<HTMLInputElement>('username').value.trim();
  const username = rawUsername.replace(/@srmist\.edu\.in$/i, '');
  const password = $<HTMLInputElement>('password').value;
  const password2 = $<HTMLInputElement>('password2').value;
  if (!username || !password || !password2) { showError('Enter your register number and both passwords.'); return; }
  ...
  try {
    ...
    $<HTMLInputElement>('captcha').focus();
  } catch {
    showError('Network error — is the server running?');
  } finally {
    btn.disabled = false;
    btn.querySelector<HTMLSpanElement>('.btn-label')!.textContent = 'Get My Wrapped';
  }
}

async function handleStep2(e: Event): Promise<void> {
  e.preventDefault();
  hideError();

  const captcha = $<HTMLInputElement>('captcha').value.trim();
  if (!captcha) { showError('Please solve the CAPTCHA first.'); return; }

  const btn = $<HTMLButtonElement>('verify-btn');
  btn.disabled = true;
  ...
  try {
    ...
    const body = await res.json() as (StudentData & { error?: string });
